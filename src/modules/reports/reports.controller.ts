...
export const getFinancialReport = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const [summary, byPayment, expenses, byService] = await Promise.all([
      ...
      pool.query(`SELECT COUNT(*)::int AS expenses, COALESCE(SUM(amount), 0)::numeric AS total FROM expenses WHERE ($1::int IS NULL OR clinic_id = $1) AND created_at >= $2 AND created_at < ($3::date + INTERVAL '1 day')`, [clinicId, from, to]),
