...
export const getOverviewReport = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const [patients, visits, appointments, prescriptions, financial] = await Promise.all([
      ...
      pool.query(`SELECT COALESCE(SUM(ii.price), 0)::numeric AS revenue, COALESCE(SUM(ii.doctor_share), 0)::numeric AS doctor_payout, COALESCE(SUM(i.discount_amount), 0)::numeric AS discounts FROM invoice_items ii JOIN invoices i ON i.invoice_id = ii.invoice_id WHERE ($1::int IS NULL OR ii.clinic_id = $1) AND i.created_at >= $2 AND i.created_at < ($3::date + INTERVAL '1 day')`, [clinicId, from, to]),
    ]);
...
export const getFinancialReport = async (req: AuthenticatedRequest, res: Response) => {
  ...
  try {
    const [summary, byPayment, expenses, byService] = await Promise.all([
      pool.query(`SELECT COUNT(DISTINCT i.invoice_id)::int AS invoices, COALESCE(SUM(i.total_amount), 0)::numeric AS gross, COALESCE(SUM(i.discount_amount), 0)::numeric AS discounts, COALESCE(SUM(i.net_amount), 0)::numeric AS net, COALESCE(SUM(i.paid_amount), 0)::numeric AS paid FROM invoices i JOIN invoice_items ii ON ii.invoice_id = i.invoice_id WHERE ($1::int IS NULL OR ii.clinic_id = $1) AND i.created_at >= $2 AND i.created_at < ($3::date + INTERVAL '1 day')`, [clinicId, from, to]),
      pool.query(`SELECT i.payment_type, COUNT(DISTINCT i.invoice_id)::int AS invoices, COALESCE(SUM(i.paid_amount), 0)::numeric AS paid FROM invoices i JOIN invoice_items ii ON ii.invoice_id = i.invoice_id WHERE ($1::int IS NULL OR ii.clinic_id = $1) AND i.created_at >= $2 AND i.created_at < ($3::date + INTERVAL '1 day') GROUP BY i.payment_type ORDER BY paid DESC`, [clinicId, from, to]),
